...
Order is a number to display the steps in the correct order. (default is `0`)<br />
...
autoHide is a boolean that allows you to toggle the default style to display the step.
Default value is `true` and will apply a `display: none` on inactive steps.

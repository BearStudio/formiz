...
import { Alpha } from '../src/messages'
...
export const MyField = (props) => {
  const { value, setValue, errorMessage, isValid, resetKey } = useField(props)
  const { label } = props
  ...
  return (
    <div>
      ...
      <input key={resetKey} type="text" defaultValue={value} onChange={(e) => setValue(e.target.value)} />
      {!isValid && (
        <p>
          { errorMessage }
        </p>
...
And we need to update the `onSubmit` on the `<form>` element to not submit all the form but just the current step.

...
**Formiz** core does not provide styles, so you can **use it with any UI library and style you want**. Use it with Bootstrap, ReactStrap, Material UI or your own styles.
...
Import the `useField` hook from `@formiz/core`
...
- Display error only if the field changed or the form/step is submitted.

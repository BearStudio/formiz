...
export const FormizDebug = ({
  store: useStore,
}: {
  store: UseBoundStore<StoreApi<Store>>;
}) => {
  const formState = useStore?.(
    (state) => formInterfaceSelector(state),
    deepEqual
  );

  const { statefields, formValues } = useStore((state) => {
      ...
      >
        <div>
          ...
          <p>
            ...
            {displayBoolean(formState.isSubmitted)} • <strong>isValid</strong>:{" "}
            {displayBoolean(formState.isValid)} • <strong>isValidating</strong>:{" "}
            {displayBoolean(formState.isValidating)} •{" "}
            <strong>isPristine</strong>: {displayBoolean(formState.isPristine)}
          </p>

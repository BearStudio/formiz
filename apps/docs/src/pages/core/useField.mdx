...
Get the current value of the field, after it has been format by formatValue (if you used it)
...
const MyField = (props) => {
  ...
  return <input value={value ?? ""} formatValue={(val) => (val || "").trim()} />;
...
- `rule(fieldValue, rawFieldValue)`: Built in validation rule or custom validation function (must returns `true` if the rule is valid).
  - `fieldValue` references the value of the field
  - `rawFieldValue` references the value of the field before any formatting of the data via `formatValue`
